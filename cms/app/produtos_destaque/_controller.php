<?php
	...
	case 'new': // Adiciona registro.
		...
		if (!empty($_POST['submit'])) {
			...
			if (empty($_SESSION['erro'])) {
				$query = mysqli_query($db, "INSERT INTO produtos_destaque (nome, referencia, codigo_barras, departamento_id, estoque, preco_custo, preco_venda, imagem, descricao, status) VALUES ('{$post['nome']}', '{$post['referencia']}', '{$post['codigo_barras']}', {$post['departamento_id']}, {$post['estoque']}, '{$post['preco_custo']}', '{$post['preco_venda']}', '{$post['imagem']}', '{$post['descricao']}', {$post['status']})");
				if ($query) {
		...
		if (!empty($_POST['submit'])) {
			...
			if (empty($_SESSION['erro'])) {
				if (!empty($post['imagem'])) {
					$query = mysqli_query($db, "UPDATE produtos_destaque set nome = '{$post['nome']}', referencia = '{$post['referencia']}', codigo_barras = '{$post['codigo_barras']}', departamento_id = {$post['departamento_id']}, imagem = '{$post['imagem']}', descricao = '{$post['descricao']}', preco_custo = '{$post['preco_custo']}', preco_venda = '{$post['preco_venda']}', status = {$post['status']} WHERE id = {$post['id']}");
				} else {
					$query = mysqli_query($db, "UPDATE produtos_destaque set nome = '{$post['nome']}', referencia = '{$post['referencia']}', codigo_barras = '{$post['codigo_barras']}', departamento_id = {$post['departamento_id']}, descricao = '{$post['descricao']}', preco_custo = '{$post['preco_custo']}', preco_venda = '{$post['preco_venda']}', status = {$post['status']} WHERE id = {$post['id']}");	
				}
				if ($query) {
	...
	case 'delete': // Deleta registro.
		...
		if (!empty($id)) {
			$query = mysqli_query($db, "DELETE FROM produtos_destaque WHERE id = {$id}");
			if ($query) {
