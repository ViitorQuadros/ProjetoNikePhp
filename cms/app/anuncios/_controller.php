<?php
	...
	case 'new': // Adiciona registro.
		...
		if (!empty($_POST['submit'])) {
			...
			if (empty($_SESSION['erro'])) {
				$query = mysqli_query($db, "INSERT INTO anuncios (nome, telefone, email, imagem, descricao, status) VALUES ('{$post['nome']}', '{$post['imagem']}', '{$post['email']}', '{$post['telefone']}', '{$post['descricao']}', {$post['status']})");
				if ($query) {
	...
	case 'edit': // Edita registro.
		...
		if (!empty($_POST['submit'])) {
			...
			// Valida imagem
			if ($arquivos->validaArquivo('imagem', 'jpeg;jpg;png;gif', 5)) {
				// Define nome do arquivo com timestamp atual.
				$post['imagem'] = time() . '.' . $arquivos->tipoArquivo('imagem');
				// Salva imagem.
				if (!move_uploaded_file($_FILES["imagem"]["tmp_name"], DIR_FILES . '/' . $post['imagem'])) {
